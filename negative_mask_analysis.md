...
**问题根源**：
- 模型在**监督训练**时只见过单掩码（`multimask_output=False`）
- 推理时使用多掩码融合可能导致**分布不匹配**
- 即使融合策略再好，也可能改变模型学习到的分布

**解决思路**：
- **保守策略**：主要使用最佳掩码，只在需要时用多掩码优化
- **避免整体融合**：不改变掩码的整体分布，只优化边界和不确定区域
- **自适应选择**：根据最佳掩码质量决定是否使用多掩码
...
### 2. 保守的多掩码优化策略 (`_adaptive_multi_mask_fusion`) ⭐ **最新版本**
...
- **高质量情况**（IoU > 0.75）：
  - **直接使用最佳掩码**，不进行融合
  - 只在边界区域进行**非常保守的优化**（5%增强）
  - 这样避免改变模型学习到的分布

- **低质量情况**（IoU ≤ 0.75）：
  - 主要使用最佳掩码（70%）
  - 在**不确定区域**（0.3-0.7且方差大）使用加权融合优化（30%）
  - 在**边界区域**使用最大值增强（8%）
  - 这样只在需要时才使用多掩码，且非常保守

#### b) 区域识别
- **边界区域**：方差大且最佳掩码值在0.5附近的区域
- **不确定区域**：最佳掩码值在0.3-0.7且其他掩码差异大的区域
- **确定区域**：直接使用最佳掩码，不改变

#### c) 保守调整
- 只在最佳掩码IoU很高（>0.75）时轻微增强（最多0.5%）
- 避免过度调整，保持与训练分布一致
...
3. **参数调优**（可选）：如果效果仍不理想，可以进一步调整：
   ...
   - `high_quality_threshold`（当前0.75）：控制何时直接使用最佳掩码
   - `boundary_enhancement_high`（当前0.05）：高质量时的边界增强系数
   - `boundary_enhancement_low`（当前0.08）：低质量时的边界增强系数
   - `uncertain_region_weight`（当前0.3）：不确定区域的融合权重

### 4. 关键改进点总结

1. **从"融合"到"优化"**：不再整体融合掩码，而是局部优化
2. **从"总是使用"到"按需使用"**：只在最佳掩码质量不够好时才使用多掩码
3. **从"改变分布"到"保持分布"**：主要使用最佳掩码，保持与训练时一致
4. **从"全局调整"到"局部优化"**：只优化边界和不确定区域，不改变整体掩码
