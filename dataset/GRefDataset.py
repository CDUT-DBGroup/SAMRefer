    ...
    def __getitem__(self, index):
        ...
        if len(self.input_ids[current_index]) == 0:
            ...
            if self.return_all_sentences and self.eval_mode:
                # 返回所有描述，用于在验证时选择最优描述
                num_sentences = len(self.input_ids[current_index])
                all_word_ids = [self.input_ids[current_index][i].clone().to(dtype=torch.long) for i in range(num_sentences)]
                all_word_masks = [self.word_masks[current_index][i].clone().to(dtype=torch.long) for i in range(num_sentences)]
                all_sentences = self.all_sentences[current_index]
